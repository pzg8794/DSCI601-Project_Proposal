...
{\large\textit{(Related Work Survey)}}}

\author{
\IEEEauthorblockN{Piter Z. Garcia}
\IEEEauthorblockA{MS Data Science / Decision-Making \& Algorithmic~Fairness\\Rochester Institute of Technology (RIT)\\pizg8794@g.rit.edu}
\and
\IEEEauthorblockN{Daniel Krutz, Travis Desell}
\IEEEauthorblockA{Department of Software Engineering, Data Science\\RIT Research Advisors\\dxkvse@g.rit.edu, tjdvse@g.rit.edu}
}
...
This survey supports a project that develops a practical, reproducible framework for
fairness-aware sequential decision-making in two target settings: clinical diagnostic
workflows and quantum-network routing. The methodological focus is a spectrum of bandit
methods ranging from non-contextual to contextual to informative contextual bandits,
designed to study how different levels of available context affect fairness, equity, and
performance under uncertainty, non-stationarity, and resource constraints.
...
side, the project focuses on routing and resource-allocation decisions under probabilistic
link success, scarce resources, and time-varying congestion, where service-equity gaps can
emerge across flow or user groups. Across both settings, the project emphasizes
time-evolving disparity reporting, baseline unfairness assessment, and fairness-aware
mitigation rather than utility-only evaluation.

Two prior works provide the concrete testbeds used in this work: a clinical
diagnostic--motivated fairness audit and mitigation pipeline (EQUITAS) in equitable bioinformatics
...
while ensuring equitable service outcomes across groups. The two-domain design is not only a
transfer study, but a way to test whether fairness-aware decision principles remain meaningful
across technically different but structurally similar sequential problems.

This survey reviews the prior work needed to support that framework, including bandit
foundations, contextual and linear bandits, learning under distribution shift and limited
context, fairness definitions, and fairness in bandit-style learning. It also includes domain
grounding for clinical disparities and quantum-network routing. The conclusion summarizes
the main research gap and clarifies how the proposed work differentiates itself by explicitly
tracking and mitigating disparities over time across both domains.
...
or different error profiles. This survey frames both settings as bandit-style decision
problems: at each step, the system must choose an action given limited context, observe
partial feedback, and balance utility with fairness over time.

In a clinical diagnostic workflow, the decision may involve which test to order next,
whether to escalate follow-up, or which model or pipeline to trust when patient information
is incomplete, delayed, or noisier for some groups. In that setting, fairness matters
because optimizing only overall accuracy can hide subgroup failures, such as false-negative
gaps or delayed care for populations whose context is systematically weaker.

In a quantum-network routing workflow, the decision may involve which path to route through
or how to allocate scarce resources under probabilistic link success and time-varying
congestion. In that setting, fairness appears as service equity: whether different flow or
user groups receive comparable routing success, latency, and access to limited quantum
resources rather than having one group consistently absorb the failures of the network.

By pairing these two settings, the project studies how different levels of context affect
fairness, equity, and performance in sequential decision-making, while making the
utility--fairness tradeoff explicit rather than treating it as a hidden side effect of
optimization.
...
is predictive of outcomes; however, when context is limited or systematically noisier for
...
In the quantum-network domain, routing decisions act as online resource allocation
...
routing outcomes, quantified by group disparities in success probability and latency across flow groups, under these
constraints. Both domains share a common mathematical structure: stochastic, history-dependent
dynamics with non-stationarity, limited context, and compounding decisions over time.
...
limited or systematically noisier for some populations. These limitations create
...
Shift / limited context (missingness + non-stationarity)
...
Garcia~\cite{garcia2025equitable_bioinformatics} presents a fairness-audit and
...
Garcia~\cite{garcia2026threat_aware_quantum_routing} evaluates contextual, adversarial,
...
\resizebox{0.9\columnwidth}{!}{%
    ...
    \node[layer, fill=gray!18, draw=gray, below=0.4cm of alloccontent] (config) {\textbf{2. CONFIGURATION}};
	    ...
	    \node[block, below=0.1cm of evaluator, font=\scriptsize] (evalcontent) {
	      Orchestrates runs
	      Captures full layered run state
	    };
...
\vspace{-0.45cm}
\caption{\footnotesize Modular framework for quantum routing evaluation, following the evaluation
...
ablation under consistent capacity and threat semantics, with full logged run state captured for downstream utility and fairness analysis.}
...
plots and winner summaries. This modular structure is reused in the proposed work so that
fairness/service equity monitoring and mitigation can be computed and applied from the same
logged run state, under identical experimental controls.
Having grounded the two experimental testbeds, the remainder of this section surveys the
algorithmic and fairness literature needed to design and evaluate a shared fairness-aware contextual bandit
...
utility/regret and do not provide fairness guarantees; when context is limited or
...
proposed setting is that context can be limited and systematically lower-quality for some
...
\subsection{Distribution shift and limited context}
...
proposed project's focus on limited context or measurement noise. The limitation is that
...
This motivates algorithm designs that remain robust when the observed context is limited
...
fairness-aware contextual bandit evaluation stack.
...
\section{Conclusion}
Prior work provides strong foundations for multi-armed bandits, contextual and linear
bandits, and evaluation under bandit feedback, as well as clear fairness definitions and
initial work on fairness constraints in bandit learning. However, a gap remains at the
intersection of: i) limited context or measurement noise, ii) time-varying conditions, and
iii) explicit, time-evolving disparity monitoring and mitigation in sequential decision
systems.

The proposed project differentiates itself by building a reusable framework that evaluates
fairness-aware bandit methods under controllable limited-context mechanisms and distribution
shift, tracks group disparities over time, and studies both clinical diagnostic workflows and
quantum-network routing within the same decision-making framework. While
Chen et al.~\cite{chen2020fair} address fair contextual MABs and Joseph
et al.~\cite{joseph2016fairness} address fairness in bandit learning, no existing work, to the
best of current knowledge, jointly evaluates outcome-based disparity metrics under
distribution shift while also testing cross-domain transfer across these two settings. In
particular, \cite{garcia2025equitable_bioinformatics,garcia2026threat_aware_quantum_routing}
serve as the two primary experimental foundations for the project’s two-domain design.

This framing matters because it treats fairness not as a post-hoc audit added after
optimization, but as part of the design and evaluation of sequential decision systems under
uncertainty, partial observability, and resource constraints.
