...
\resizebox{0.9\columnwidth}{!}{%
    ...
    \node[layer, fill=gray!18, draw=gray, below=0.4cm of alloccontent] (config) {\textbf{2. CONFIGURATION}};
	    ...
	    \node[block, below=0.1cm of evaluator, font=\scriptsize] (evalcontent) {
	      Orchestrates runs
	      Logs run artifacts \& per-layer state
	    };
...
ablation under consistent capacity and threat semantics, with run artifacts and per-layer state captured for downstream utility and fairness analysis.}
...
run artifacts and per-layer state, under identical experimental controls.
