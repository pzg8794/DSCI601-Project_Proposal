...
two complementary routing families. The first is a quantum-routing testbed
...
abstraction is that each domain routes scarce resources from partial feedback
while fairness risk emerges from unequal context quality.

Four research questions drive the work. RQ1 asks how much richer context
improves utility and fairness relative to non-contextual baselines. RQ2 asks
how quickly disparities emerge when a group, site, cohort, or flow class
receives noisier, delayed, or incomplete context. RQ3 asks whether
missingness-aware or fairness-mediated policy updates reduce outcome gaps
without unacceptable utility loss. RQ4 asks whether the same fairness
mediation strategy transfers between clinical diagnostic simulation and
quantum routing, or whether mitigation must remain domain-specific.
Preliminary evidence supports the context-gain premise: predictive-context
quantum policies improve over no-context baselines by 3.74 percentage points
in the internal corpus and 2.83 percentage points in the standardized external
corpus, while the clinical EQUITAS evidence package shows large improvements
after domain context restoration and under-represented-context augmentation.
...
analysis will expand this discussion with direct comparisons once the
experiments are complete, but the literature already establishes the
...
The implementation realizes the method in Table~\ref{alg:equitas_bandit}
through a modular experiment architecture. Each environment supplies domain
state, available actions, outcome semantics, and context-quality indicators.
A runner executes one or more policy models inside that environment. A
resource allocator controls scarce capacity before actions are selected, such
as qubit/path capacity in quantum routing or test/retest/escalation capacity
in the clinical setting. An evaluator coordinates repeated runs, collects
states, and computes utility, regret, and disparity summaries. EQUITAS sits
between policy scoring and evaluation: it can mediate action scores during a
run, or it can audit saved states after a legacy run without changing the
...
Table~\ref{tab:implementation} summarizes the core components. The design is
intentionally object-oriented because clinical and quantum experiments share a
control pattern but not the same domain semantics. The quantum environment
defines network topologies, link uncertainty, path decisions, allocator
settings, and routing outcomes. The clinical environment defines sites,
cohorts, service lines, diagnostic actions, resource constraints, and
diagnostic outcomes. Both environments emit comparable state records so the
same evaluator and fairness reporting strategy can operate across domains.

\subsection{State Persistence and Reporting}
The system treats state as a first-class research artifact. Model state
captures policy-level information needed to resume or inspect a learner.
Runner state captures the sequence of environment interactions for a specific
configuration, seed, and allocator setting. Evaluator state captures
cross-run summaries needed for comparing policies and scenarios. Reporting
state captures the derived datasets, fairness summaries, visualizations, and
manifests needed to reproduce claims. These layers prevent a long-running
experiment from becoming a single opaque metric and allow fairness analyses to
be repeated as definitions or mitigation strategies evolve.
...
workflow is an evidence-flow view: domain context enters through an environment
abstraction, runner/model/allocator decisions produce stateful outcomes, the
EQUITAS layer converts those outcomes into utility and fairness evidence, and
the reporting layer emits datasets, figures, manifests, and reproducibility
artifacts. The same flow supports clinical simulation and quantum routing
without forcing either domain to imitate the other.}
\label{fig:architecture}
\end{figure*}

\subsection{Validation Strategy}
Validation is organized around contracts rather than ad hoc scripts. Stage
tests verify that data collection, cleaning, preparation, modeling,
evaluation, and reporting artifacts satisfy their schemas. Experiment tests
verify that clinical and quantum runs emit non-empty state records with the
fields required for utility and fairness analysis. Sidecar tests verify that
legacy quantum states can be converted into canonical fairness outcomes
without changing old execution behavior. Reporting tests verify that master
datasets, figures, and manifests remain linked to their source states. This
strategy connects software correctness to scientific reproducibility: the
same state contracts that protect code changes also preserve the evidence
needed to interpret preliminary findings.
