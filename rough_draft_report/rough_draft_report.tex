...
\begin{tikzpicture}[
  ...
  block/.style={rectangle, rounded corners, draw, thick, align=center,
    text width=2.75cm, minimum height=0.8cm, font=\scriptsize},
  arrow/.style={-Stealth, thick}
]
\node[block, fill=phaseblue!15, draw=phaseblue] (env) {\textbf{Environment}\\Clinical workflow\\Quantum network};
\node[block, fill=phasegreen!15, draw=phasegreen, right=of env] (context) {\textbf{Context Observation}\\complete, noisy,\\missing, delayed};
\node[block, fill=phaseorange!15, draw=phaseorange, right=of context] (policy) {\textbf{Routing Decision}\\MAB, CMAB,\\iCMAB policy};
\node[block, fill=phasepurple!15, draw=phasepurple, right=of policy] (eval) {\textbf{Evaluation}\\utility, regret,\\fairness gaps};
\draw[arrow] (env) -- (context);
\draw[arrow] (context) -- (policy);
\draw[arrow] (policy) -- (eval);
\draw[arrow, bend left=35] (eval.north) to node[above, font=\scriptsize] {policy update / mitigation} (context.north);
\end{tikzpicture}}
\caption{Shared domain model. Clinical diagnostics and quantum routing are
treated as sequential routing problems in which missing or degraded context
can create time-evolving utility and fairness failures.}
