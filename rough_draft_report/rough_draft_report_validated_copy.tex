...
\centering
\includegraphics[width=\linewidth]{presentation/clinical_routing_context_comparison.jpeg}
\vspace{0.2em}
\textbf{(a) Clinical diagnostic routing}
\end{minipage}
...
\begin{minipage}{0.92\textwidth}
\centering
\includegraphics[width=\linewidth]{presentation/quantum_routing_context_comparison.jpeg}
\vspace{0.2em}
\textbf{(b) Quantum network routing}
\end{minipage}
\caption{Motivating routing examples for the shared problem. In both panels,
the left side shows weak-context or baseline allocation and the right side
shows context-aware fairness allocation. The clinical example shows how
context-aware routing can shift testing capacity toward higher-burden or
under-served communities. The quantum example shows the analogous shift from
baseline routing toward fairness-aware iCMAB allocation for higher-demand or
less reliable flow pairs. Together, the examples visualize the paper's core
claim: scarce resources are routed under partial feedback, and context quality
shapes who absorbs the worst errors.}
\label{fig:intro_routing_examples}
\end{figure*}

Figure~\ref{fig:intro_routing_examples} summarizes this shared routing pattern.
