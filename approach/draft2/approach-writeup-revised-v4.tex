...
This shared root cause makes the two domains analytically equivalent and
...
of missing-context routing positions this work at the intersection of present
...
The COVID-19 pandemic provides the clearest real-world instance of what
this model is designed to study. Diagnostic tests---scarce, unevenly routed,
and delayed---failed the communities bearing the highest risk, producing
measurably worse infection, hospitalization, and mortality rates among Black,
Latino, and other minority groups, confirming that unfair routing is a
performance failure, not only an equity concern.

What makes this framework a genuine breakthrough is that both domains share
...
\subsection{The Approach and its Uniqueness}
No prior work evaluates fairness-aware sequential routing under a unified
bandit framework spanning clinical and quantum domains, and no prior work has
identified missing context as the shared structural root cause of routing
unfairness across both. Clinical bandit research optimizes treatment decisions
without examining how missing population context degrades routing equity;
quantum routing research maximizes throughput without connecting context
missingness to service-equity disparities. Studying them together produces
findings that improve today's clinical systems and inform fair routing in
tomorrow's quantum-enabled infrastructures.

\subsection{The Approach and its Benefits}
...
iCMAB spectrum:} each directly operationalizes the core
...
testbed and the quantum environment extension. The simulation introduces missing and group-unequal context as a controlled variable, making
...
mitigation mechanism is evaluated to enable policy comparison
that ignore, partially use, or actively recover missing context.
...
and builds the first fairness-aware sequential decision framework evaluating
both under identical conditions. Second, it provides the first systematic
...
human health impact to a proven breakthrough technology through their shared
