...
properly captured. \textit{Phase~2 is therefore not a passive observation
step; it is where the root cause of routing unfairness lives}---and the
primary phase this framework is designed to interrogate. \textit{Phase~4} then
...
missing and group-unequal context as a controlled variable, making \textsc{Phase~1}
a research instrument rather than just a data source. The quantum extension
means \textsc{Phase~1} inherits domain-realistic conditions---link success rates,
entanglement scarcity, threat signals, reproducing the same
missing-context failure mode without creating a testbed.
...
root cause; spanning three levels helps directly measure how much of the routing 
...
the only structurally honest choice given what \textit{Phase~4} can return.
...
current systems and a foundation for next-generation quantum-enabled 
infrastructures, where fair and high-performing routing are understood as the 
same problem.
...
\begin{tikzpicture}[baseline=-0.5ex, font=\scriptsize]
  ...
  \node[right=1.15cm, font=\scriptsize\bfseries, text=phase1blue]
    at (0,0) {Zoom-in on \textbf{Environment / Testbed} node};
\end{tikzpicture}
\vspace{0.4em}
...
\caption{%
  ...
  \textbf{(b)}~Zoom-in on the double-bordered \textcolor{phase1blue}{\textbf{Environment / Testbed}}
    node, detailing the six-layer modular pipeline.
